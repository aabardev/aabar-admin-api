...
export const saveMenuItem = async (data) => {
  ...
  // (2) Save data to db
  res = await db.upsertMenuItem(eData);
  console.log("RES:", res);
  if (res.status === "success" ) {
    return { status: res.status, msg: "Menu Item saved successfully" };
  } else {
    return {
      status: res.status,
      error: res.error,
      msg: "Could not save Menu Item",
    };
  }
};
...
export const saveOffer = async (data) => {
  ...
  // (2) Save data to db
  res = await db.upsertOffer(eData);
  console.log("RES:", res);
  if (res.status === "success" ) {
    return { status: res.status, msg: "Offer saved successfully" };
  } else {
    return {
      status: res.status,
      error: res.error,
      msg: "Could not save Offer",
    };
  }
};
...
export const saveEvent = async (data) => {
  ...
  // (2) Save data to db
  res = await db.upsertEvent(eData);
  console.log("RES:", res);
  if (res.status === "success" ) {
    return { status: res.status, msg: "Event saved successfully" };
  } else {
    return {
      status: res.status,
      error: res.error,
      msg: "Could not save Event",
    };
  }
};
